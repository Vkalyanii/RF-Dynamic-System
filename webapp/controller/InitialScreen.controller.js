...
sap.ui.define([
    ...
    function (Controller, PDFViewer, JSONModel, Device, MessageToast, MessageBox, Filter, FilterOperator) {
        ...
        return Controller.extend("com.app.userrouting.controller.InitialScreen", {
            ...
            onFinishconnectSAPPress: function () {
                ...
                oModel.read("/ServiceSet", {
                    ...
                    success: function (oData) {
                        ...
                        var errorMessages = [];
                        var t = oData.results[0].Sid;
                       /***commented code on 14-10-2024 */ 
                        ...
                        // };
                        var oEntry = { Sid : t,Resourceid :this.ID }
                        // Save to OData service
                        oModel.create("/RelationSet", oEntry, {
                            success: function () {
                    ...
                    onClose: function (status) {
                        if (status === MessageBox.Action.DELETE) {
                            ...
                            this.arrayOfClient.forEach(element => {
                                ...
                                oModel.remove(`/RelationSet(Sid='${oUssid}',Resourceid='${oUser}')`, {
                                    success: function () {
            ...
            oModel.read("/RelationSet",
                {success:function(odata){
                    console.log("Raw OData Results:", JSON.stringify(odata, null, 2)); // Log the entire response

                    // Filter the results based on Resourceid
                    var filteredResults = odata.results.filter(entry => {
                        // Log values and lengths for debugging
                        console.log("ouser value:", ouser, "Length:", ouser.length);
                        console.log("Resourceid value:", entry.Resourceid, "Length:", entry.Resourceid.length);
                
                        // Trim and normalize both values before comparison
                        var normalizedOuSer = ouser.trim().toLowerCase();
                        var normalizedResourceid = entry.Resourceid.trim().toLowerCase();
                
                        // Log character codes for deeper debugging
                        function logCharacterCodes(str) {
                            return str.split('').map(char => char.charCodeAt(0)).join(', ');
                        }
                        
                        console.log("ouser char codes:", logCharacterCodes(normalizedOuSer));
                        console.log("Resourceid char codes:", logCharacterCodes(normalizedResourceid));
                
                        // Return the comparison result
                        return normalizedResourceid === normalizedOuSer;
                    });
                
                    console.log("Filtered Results:", filteredResults); // Log filtered resultss
